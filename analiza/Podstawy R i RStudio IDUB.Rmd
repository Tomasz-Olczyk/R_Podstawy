...
Dlatego używamy RMakdown. Jest to czysto tekstowy dokument, który umożliwia robienie tych wszystkich rzeczy, w sposób reprodukowalny i zapisany w możliwie odporny sposób.
...
Zbiory danych zaimportowane do  R też są obiektami. W R jest kilka typów obiektów służących do przechowywania danych ale najbadziej typowym jest ramka danych: data frame. Ramka danych to prostokątna tabela zlożona z wierszy (obserwacji) i kolumn (zmiennych). Kolumny w ramce danych mogą być różnej klasy.
...
```{r}
panteon <- read_csv(here::here("dane", "panteon_s.csv"))
```
...
podatki <- read_excel(
  "./dane/Chapter14TablesFigures.xlsx", #uwaga musimy być we właściwym katalogu roboczym, żeby ta linia kodu zadziałała
...
```{r}
podatki_open <- openxlsx::read.xlsx("./dane/Chapter14TablesFigures.xlsx", #ścieżki relatywne
                          sheet = "TS14.1",
                          startRow = 4,
                          rows = 1:118,
                          cols = 1:5) 
