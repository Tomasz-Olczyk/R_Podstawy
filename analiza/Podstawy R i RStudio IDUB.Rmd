---
title: "Podstawy R i RStudio - moduł 1"
...
knitr::opts_chunk$set(
  echo = TRUE,             # Pokazuje kod R w dokumencie (jeśli FALSE – kod jest ukryty)
  error = TRUE,            # Jeśli wystąpi błąd, nadal kontynuuje renderowanie i wyświetla błąd w dokumencie
  warning = FALSE,         # Ostrzeżenia nie będą wyświetlane w dokumencie
  fig.path = (here::here("wykresy/"))    # Ścieżka do folderu, w którym zapisywane będą wszystkie wygenerowane wykresy
)
```
...
Wszystkie wyniki (tabele, wykresy itp) powinny byc procedualnie i reprodukowalnie generowane z zapisanego kodu, zapisane w prostym formacie opartym na tekście. Możliwość reprodukowania własnej pracy jest kluczowa w nauce, ale także ze względów czysto praktycznych. Osobą, która będzie najczęściej reprodukować Twoje wyniki jesteś Ty za sześć miesięcy.

Celem jest maksymalnie programatyczna analiza. Staramy się nie używać sposobów, które nie są programatycznie reprodukowalne.
...
znak `#` oznacza komentarz wewnętrz segmentu kodu.WRmarkdown odnosi się do poziomu nagłówka.
...
Typowe błędy: 
- niedomknięty nawias lub cudzysłów
- brak przecinka między argumentami
- niekompletne wyrażanie (+ w konsoli, kursor + Esc)
- częsty błąd ggplot to + w niewłaściwm miejscu
...
Alt - = operator przypisania w Windows
Opt - = operator przypisania w Mac os
...
możliwości bazowego R
 ...
 Pliki R markdown składają się z nagłówka, segmentów tekstu i specjalnie oznaczonych segmentów kodu(code chunk). Skrypty składają się z kodu i komentarzy poprzedzonych #. W markdown "domyślną" treścią jest tekst "do czytania", kod jest dodatkiem, dlatego znajduje się w specjalnie zaznaczonych segmentach. Raportom w R markdown poświęcimy więcej czasu w drugim segmencie szkolenia.
...
Poniższy segment jest nazywany code chunk:
...
zmienne, dane, funkcje.

Do nadawania nazw nie wolno używać m.in: FALSE, TRUE, Inf, for, else, break, function. Lepiej nie używać we własnych nazwach nazw funkcji, szczególnie z pakiety podstawowego jak c() - konkatentacja, q(), mean(), var(), range() i podobnych.
...
Zbiory danych zaimportowane do  R też są obiektami. W R jest kilka typów obiektów służących do przechowywania danych ale najbardziej typowym jest ramka danych: data frame. Ramka danych to prostokątna tabela zlożona z wierszy (obserwacji) i kolumn (zmiennych). Kolumny w ramce danych mogą być różnej klasy.
...
logiczne <- two == Two # podwójny znak = oznacza w R równa się pojedynczy znak = jest w R operatorem przydzielania oprator przypisania może być skierowany w prawo ->
...
Inaczej niż w Pythonie i wielu innych języków programowania R indeksuje od 1 a nie od zera
...
Matryca (macierz) z dwóch wektorów
...
Ramka danych (data frame) z dwóch wektorów:

```{r}

ramka_danych <- tibble(partie, sondaż)
...
Operator $ pozwala wybrać kolumnę z ramki danych
...
Podstawowe funkcje do wczytywania danych:
...
Do R można wczytać (a także eksportować z R) dane w bardzo wielu różnych formatach, m.in dane wyjściowe: SPSS, STATA, SAS. W przypadku wymienionych (a także innych formatów) odpowiednie funcje zawierają np. biblioteki foreign [link](https://cran.r-project.org/web/packages/foreign/index.html) i haven [link](.https://haven.tidyverse.org/)

Dane z arkuszy google można wczytywać funkcją biblioteki googlesheets4 

Można również zczytywać tabele z danymi ze stron internetowych za pomocą biblioteki rvest (np. z wikipedii) czy z plików pdf za pomocą biblioteki tabulizer.


Praktycznie każde działanie w R można wykonać co najmniej na kilka sposobów za pomocą różnych funkcji różnych pakietów. Dotyczy to zarówno analizy, grafiki jak i wczytywania plików.
...
Problem ścieżek absolutnych i relatywnych

```{r}

podatki <- read_excel(here("dane", "Chapter14TablesFigures.xlsx"), #uwaga musimy być we właściwym katalogu roboczym, żeby ta linia kodu zadziałała
...
```{r}
podatki_2 <-read.xlsx(here::here("dane",  "Chapter14TablesFigures.xlsx") )
```

Dlaczego pojawia się komunikat o błędzie?
...
```{r}
podatki_open_2 <- openxlsx::read.xlsx(here::here("dane", "Chapter14TablesFigures.xlsx"), #ścieżki relatywne
                          sheet = "TS14.1",
                          startRow = 4,
                          rows = 1:118,
                          cols = 1:5) 
...
```{r}
glimpse(podatki_open_2)

podatki_open_2$X1 <- as.numeric(podatki_open_2$X1)
```
...
```{r}
panteon_lokalnie <- read_csv(here("dane", "panteon_s.csv"))
```
...
```{r}
length(podatki_open$U.S.)
```

```{r}
length(podatki_open$X1)
```
...
```{r}
write_csv(podatki_open, here::here("dane", "podatki_lepsze.csv"))
```

```{r}
podatki_nowe <- read_csv(here::here("dane", "podatki_lepsze.csv"))
```
...
#### Korekta marginesów wykresu w bazowym R
...
## Praca domowa 1 (do modułu 1):

- tworzymy nowy projekt
- umieszczamy w nim plik z danymi
- tworzymy w nim plik r markdown który wczytuje potrzebne biblioteki,
wczytuje dane i rysuje na ich podstawie (dowolny) wykres
...
Informacje o pakietach R według dziedzin:
