...
Jak już wiemy dane xlsx są problematyczne:


```{r}

podatki <- read_excel(here("dane", "Chapter14TablesFigures.xlsx"), 
  sheet = "TS14.1", #wybieramy arkusz z pliku
  skip = 3, # pomijamy trzy pierwsze wiersze
  n_max = 114 # kończymy na 114 obserwacji
  )
...
Argument mapping zawsze wystepuje z funkcją aes(). aes() łączy zmienną z elementem, który będzi widoczny na wykresie. Inaczej mówiąc mapowanie mówi które zmienne będą reprezentowane przez które elemenety wizualne.
...
Kolejne geometrie to kolejne warstwy wykresu. 
...
Mamy wiele różnych estetyk do wyboru dla każdej geometrii. Każda nowa estetyka generuje własną legendę.
...
Poniższy kod wybiera z danych Panteon, te które

```{r}
pol <- subset(panteon, countryCode3 == "POL")
```

Geometrie mogą przyjmować różne dane, co otwiera ciekawe możliwości:
...
ggsave("my-plot.pdf") # > zapisuje 6 x 3.71 cali
...
- uwaga do wersji trudniejszej: żeby użyć facet_wrap na tych danych trzeba zamienić dane podatki na format long funkcją tidyr::pivot_longer [opis tutaj](https://tidyr.tidyverse.org/reference/pivot_longer.html)
...
[Ściągawka R Markdown](https://rstudio.github.io/cheatsheets/html/rmarkdown.html)
...
Rozdział 16 Środowisko R Markdown i publikowanie w: Long, J. D.  (2020). Język R: Receptury: analiza danych, statystyka i przetwarzanie grafiki, (K. Sawka, Tłum.). Helion SA.

[to samo online po angielsku](https://rc2e.com/rmarkdown)
