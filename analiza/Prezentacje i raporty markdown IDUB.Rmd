...
Wykonanie kodu w notatniku sprawia że rezultaty pojawiają się poniżej kodu.
...
Kod dodajemy *Insert Chunk* na pasku narzędzi albo skrótem *Cmd+Option+I* w Mac OS  
w windows *Ctdl+Option+I* 
...
chunk label (n.p, my-chunk ). Tylko ona nie ma taga ., my-chunk, albo label = "my-chunk" Te etykiety muszą być unikalne, segementom bez numeru przypisywane są etykiety takie jak unnamed-chunk-i, gdzie i jest rosnącą liczbą. Ma to znaczenie np. w sytuacji kiedy kod w segmencie tworzy wykres. W takim przypadku plik graficzny zawierający wykres ma nazwę taką jak segment kodu.
...
-   `eval`(`TRUE`) czy wykonać fragment kodu czy nie, można takze podać, które wyrażenia wykonać za pomocą wektora liczb `eval = c(1,3)`
